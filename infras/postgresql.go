...
func CreateDBConnection(name, host, port, dbName, username, password, timeZone string) *sqlx.DB {
	descriptor := fmt.Sprintf(
		...
		password,
		dbName)
	if os.Getenv("SERVER.ENV") == "development" {
	...
	if err != nil {
		if os.Getenv("SERVER.ENV") == "development" {
			log.Fatal().Err(err).Str("name", name).Str("host", host).Str("port", port).Str("dbName", dbName).Msg("Failed connecting to database")
		}
	} else {
		if os.Getenv("SERVER.ENV") == "development" {
			log.Info().Str("name", name).Str("host", host).Str("port", port).Str("dbName", dbName).Msg("Connected to database")
		}
	}
...
func CreateGormConn(name, host, port, dbName, username, password, timeZone string) *gorm.DB {
	...
	if err != nil {
		if os.Getenv("SERVER.ENV") == "development" {
			log.Fatal().Err(err).Str("Gorm->name", name).Str("host", host).Str("port", port).Str("dbName", dbName).Msg("Failed connecting to database")
		}
	} else {
		if os.Getenv("SERVER.ENV") == "development" {
			log.Info().Str("Gorm->name", name).Str("host", host).Str("port", port).Str("dbName", dbName).Msg("Connected to database")
		}
	}
	sqlDB, err := db.DB()
